...
fork(void)
{
  ...
  np->vruntime = np->parent->vruntime;
  np->map_count = np->parent->map_count;
  *np->tf = *curproc->tf;
  if(np->map_count>0)
  {
      mmap_fork(curproc,np);
  }
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if(p->state==RUNNABLE) {
            ...
            if(p->over == choosen->over)
            {
                if((p->vruntime) < (choosen->vruntime)) choosen=p;
            }
            else
            {
                if(p->over < choosen->over) choosen=p;
            }
            //if(p->vruntime < choosen->vruntime) choosen=p;
        }
      }
...
void ps(int pid){
    ...
    if(pid==0){
        ...
        for(p=ptable.proc;p < &ptable.proc[NPROC]; p++)
        {
            ...
            {  
                if(p->over>0) {
                    if(p->execTime>=10000000)
                    cprintf("%s\t%d\tEMBRYO  \t%d\t\t%d\t\t%d\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over);
                    else
                    cprintf("%s\t%d\tEMBRYO  \t%d\t\t%d\t\t%d\t\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over); 
                } else{ 
                    if(p->execTime>=10000000)
                    cprintf("%s\t%d\tEMBRYO  \t%d\t\t%d\t\t%d\t%d\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime);
                    else 
                    cprintf("%s\t%d\tEMBRYO  \t%d\t\t%d\t\t%d\t\t%d\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime);
            ...
            else if(p->state==SLEEPING)
            {
                if(p->over>0) {
                    if(p->execTime>=10000000)
                    cprintf("%s\t%d\tSLEEPING  \t%d\t\t%d\t\t%d\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over);
                    else
                    cprintf("%s\t%d\tSLEEPING  \t%d\t\t%d\t\t%d\t\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over); 
                } else{ 
                    if(p->execTime>=10000000)
            ...
            else if(p->state==RUNNABLE)
            {
                if(p->over>0) {
                    if(p->execTime>=10000000)
                    cprintf("%s\t%d\tRUNNABLE  \t%d\t\t%d\t\t%d\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over);
                    else
                    cprintf("%s\t%d\tRUNNABLE  \t%d\t\t%d\t\t%d\t\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over); 
                } else{ 
                    if(p->execTime>=10000000)
            ...
            else if(p->state==RUNNING)
            {
                if(p->over>0) {
                    if(p->execTime>=10000000)
                    cprintf("%s\t%d\tRUNNING  \t%d\t\t%d\t\t%d\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over);
                    else
                    cprintf("%s\t%d\tRUNNING  \t%d\t\t%d\t\t%d\t\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over); 
                } else{ 
                    if(p->execTime>=10000000)
            ...
            else if(p->state==ZOMBIE)
            {
                if(p->over>0) {
                    if(p->execTime>=10000000)
                    cprintf("%s\t%d\tZOMBIE  \t%d\t\t%d\t\t%d\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over);
                    else
                    cprintf("%s\t%d\tZOMBIE  \t%d\t\t%d\t\t%d\t\t%d+%d*2147483647\n",name,p->pid,p->priority,p->execTime/weightInfo[p->priority],p->execTime,p->vruntime,p->over); 
                } else{ 
                    if(p->execTime>=10000000)
        ...
        for(p=ptable.proc;p<&ptable.proc[NPROC];p++)
        {
            if(p->pid==pid)
            {
                ...
                cprintf("\nname\t\tpid\tstate\t\tpriority\truntime/weight\texecTime\tvruntime\tticks : %d\n",ticks);
                if(p->state==EMBRYO)
                {  
                    if(p->over>0) {
                        if(p->execTime>=10000000)
...
int setnice(int pid, int priority){
    ...
    struct proc *p;
    if(priority<0 || priority>39) return -1;
    acquire(&ptable.lock);
    for(p=ptable.proc;p<&ptable.proc[NPROC];p++)
    {
        if(p->pid==pid&&p->state!=UNUSED){
