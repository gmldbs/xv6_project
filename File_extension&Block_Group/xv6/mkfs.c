...
int nbitmap = ((FSSIZE%(BSIZE*8))==0)? FSSIZE/(BSIZE*8) : FSSIZE/(BSIZE*8) + 1;
...
void balloc(int used)
{
  ...
  wsect(sb.nmeta+sb.ninodeBG, buf);
}
  ...
  while(n > 0){
    ...
    if(fbn < NDIRECT){
      if(xint(din.addrs[fbn]) == 0){
        din.addrs[fbn] = xint(sb.nmeta + sb.metaBG + freeblock++);
      }
      x = xint(din.addrs[fbn]);
    } else {
      if(xint(din.addrs[NDIRECT]) == 0){
        din.addrs[NDIRECT] = xint(sb.nmeta + sb.metaBG + freeblock++);
      }
      rsect(xint(din.addrs[NDIRECT]), (char*)indirect);
      if(indirect[fbn - NDIRECT] == 0){
        indirect[fbn - NDIRECT] = xint(sb.nmeta + sb.metaBG + freeblock++);
