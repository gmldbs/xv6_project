...
// to find minimum active bgroup // but not ueed
/*int min_bgroup(){
    struct buf *bp;
    int minBgroup=0, min=sb.bgroupsize,m;
    int dev=1;
    for(int i=0;i<sb.nblockgroups;i++) 
    {
        int firstblock = BBLOCKGROUPSTART(i,sb);
        int num=0;
        for(int b=firstblock+sb.bgroupmeta;b<firstblock+sb.bgroupsize;b+=BPB) 
        {
            bp = bread(dev,BBLOCK(b,sb));
            for(int bi=0;bi<BPB && b+bi<sb.size;bi++)
            {
                m = 1<<(bi%8);
                if((bp->data[bi/8] & m) == 1) num++;
            }
            brelse(bp);
        }
        if(num < min) 
...
// to same distribution // but not need 
/*static uint try_balloc_same(uint dev, uint bgroupnum)
{
    int b, bi, m;
    struct buf *bp;
    int firstblock = BBLOCKGROUPSTART(bgroupnum, sb);
    for(b=firstblock+sb.bgroupmeta;b<firstblock+sb.bgroupsize;b+=BPB) {
        bp = bread(dev, BBLOCK(b,sb));
